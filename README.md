...
One foreground belongs here too. The gutter — line numbers, listchars, indent guides — is every glyph that is neither content nor a comment, and it was the one colour a renderer mixed by hand. It drifted into exactly the collision the contract exists to catch: at a 0.22 blend it landed on `#7a7e7b`, **0.036** from `comment` at `#7e8a7b`, so the number column and the comments on the same screen wore one colour. Nothing measured it, because the mix happened in render.py. It is now `GUTTER_LIFT`, solved like a surface and audited like a role: `#646764`, 3.5:1 against the ground and 0.111 from `comment`.
...
| lualine | `lualine/fmind.lua` | Copy into `~/.config/nvim/lua/lualine/themes/`. No configuration |

Everything else reads the terminal's own colours and is themed for free: `bat --theme=ansi`, lsd, lazygit, lazydocker, bottom, atuin, yazi, fastfetch, gh, ripgrep, jq. Keeping them on ANSI is deliberate — one Ghostty line reskins the lot. What that costs is the bright half: a tool reading slot 12 gets `parameter` whatever it meant by "bright blue". It also costs slot 2, which is `string` and therefore a white: a tool that asks for "green" gets white, and the ones that mean *added* or *ok* by it get white too.

Inside the theme that is `SEMANTIC`, and it is named rather than remembered because remembering it failed four times. The Neovim `Added` group, opencode's `success` and `diffAdded`, and ptpython's `generic.inserted` all reached for `string` because it is the slot called green, so a plus row arrived at chroma 0.003 next to a red minus row — and gitsigns compounded it, deriving its staged tier by dimming `Added` and landing staged-additions on the neutral grey `#6b6c6b`. A renderer now asks for the meaning, `p.semantic("added")`, and cannot reach the wrong slot. Outside the theme the fix belongs in the consumer and it is one line: point whatever the tool calls *ok*, *added* or *small* at slot 4 rather than slot 2. An `ls` listing whose new files are white is a config bug, not a palette one.
...
It also has to set the groups Neovim would otherwise answer itself, because `highlight clear` does not clear them — it restores them. A group left out is not inherited from `Normal`; it is set to one of Neovim's own slate blues and it stays there. `NonText` was the expensive omission: **twenty-seven** groups across four plugins link at it, so one missing line put Copilot's ghost text, the LSP inlay hints, the blame virtual text and the directory half of every picker row at `#4f5258` — 2.5:1, at hue 264, which is outside every hue window this contract admits. `CurSearch`, `MoreMsg`, `TabLine` and the four `Spell*` groups were the same kind of hole, and neotest, grug-far and todo-comments were inventing colours of their own: a failed test at `#f70067`, an add indicator at 2.4:1, one `#d1242f` for both *changed* and *removed*, and `PERF` and `TEST` comments both painted as plain code because todo-comments derives them from `Identifier`.

The way to find them is to dump every resolved group with the whole plugin stack loaded and check each colour against the palette. That took the off-palette count from **59 to 24**, and of the 24 all but one belong to something that is not the theme's to set: GitHub's own brand hex under Octo, bufferline's in-family derivations, and Neovim's internal error groups. The one exception is `VenvSelectActiveVenv`, which venv-selector assigns after the colorscheme has loaded.

The git gutter is where that mattered most, and it needed one more idea. LazyVim gives staged and unstaged hunks the same sign glyph, so colour is the only thing separating them — and gitsigns derives the staged tier by dimming the unstaged one a flat 30%, which landed staged-delete at **1.9:1** and staged-change at 2.4:1. `recede()` solves for a contrast instead of guessing a weight, the way the surfaces solve for a lift, and `RECEDED_CONTRAST` is 3.5 rather than 4.0 because `deleted` decides it: `error` already sits at 5.30:1, so at 4.0 its receded tier is only 0.076 away and the two tiers stop being two. The `Nr` and `Cul` variants are spelled out too, because gitsigns builds those from its own colours rather than from the base group.
...
A multiplexer's status bar and an editor's statusline are the two pieces of chrome that are on screen every second, and both were the last thing in the theme to actually get the palette.

Zellij's bottom bar had **no green in it at all**. The named block spends the body green on `ribbon_selected`, and in base mode that ribbon is never down there — it is the active tab, at the top. What the status bar paints is eight mode names from `ribbon_unselected` and eight key chips from `emphasis_3`, so those two entries are the whole bar: it read as grey labels at 4.6:1 with teal keys and an orange overflow. The mode names are now chalk at **6.7:1**, and `emphasis_3` is the body green, because the key you press is the one thing on that row you act on.

That exposed the accent quartet as well. A bar labels four kinds of thing and hands them to `emphasis_0..3`, so those four need the separation two adjacent syntax roles need — and the first quartet was `number`, `type`, `escape`, `property`: three cool, and two of them the same teal at 0.119 and 0.127, under the 0.130 floor. A bar that looked like it carried four kinds of information carried three. `EMPHASIS` is now `number`, `type`, `builtin`, `variable`: orange, cyan, violet, green, closest pair **0.264**.

lualine had the opposite problem — it never read the theme at all. On `theme = "auto"` it *derives* one, and what it derives here is wrong three ways at once. The normal-mode chip is the background of `PmenuSel`, so the mode you are in almost all the time was the only one with no colour: `#7b7f7b` on `#2f302f`, **3.3:1**, grey on grey. The insert chip is the foreground of `String` — a near-white here — lightened another 10% to `#ebedeb` at **16.8:1**, brighter than anything the contract admits, since `string` is capped at 15:1 for exactly this reason. And every colour it passes through gets that 10% brightener, so the section grounds landed on `#080c08` and `#1f221f`: one and two hex off the real ground and panel, which reads as a seam. The diff counters were its own `#90ee90`, `#f0e130` and `#ff0038`, because it looks up `LuaLineDiffAdd` and `GitSignsAdd` first and neither was set.

None of that needed configuration to fix. `auto` looks for `lua/lualine/themes/<colors_name>.lua` on the runtimepath before it derives anything, and sorts a file under the user's config directory first — so `lualine/fmind.lua` replaces the derivation outright. The mode is carried by the chip's *text* on the selection surface rather than by a lit block, which is the same decision the Zellij ribbons make and for the same reason: a filled rectangle at 16:1 for the whole session is the loudest thing on the screen, and it is chrome. Six modes, six roles, closest pair 0.162.
...
Six levels now get six colours rather than six weights of one, and which six is `HEADING_LADDER`'s decision rather than render.py's, because a set of colours that appear in one document is a contract question. The first ladder was picked by eye and measured nothing: it put a teal at level five, **0.119** from the cyan at level two and **0.121** from the chalk at level six, and it ended on the same chalk that paints the bullets, the quote bars and the table rules in the same buffer — so a level-six heading was indistinguishable from a table row. It is now body green, amber, cyan, orange, violet, the darker green: every pair at least **0.191** apart, and prominence falling at every step, so the ladder can be skimmed by lightness and not only by hue.

Only the top two get a bar, at the neutral `heading1` and `heading2` luminances above. Below that the ladder carries the level on its own; a bar on every heading is noise. Inline code is `escape` on the panel ground, links are `parameter` and underlined, bullets, quotes and table rules are chalk, and a checked box is `function`.
